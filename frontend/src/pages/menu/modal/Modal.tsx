...
const Modal = ({
	...
	setModalOpen: (change: boolean) => void;
}) => {
	return (
		<>
			<div
					...
					<div className="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
						<div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
							<div className="sm:flex sm:items-start">
								<div
									className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-100 sm:mx-0 sm:h-20 sm:w-20"
									style={{ background: `url(${jaffa_burger}) center/cover` }}
								></div>
						...
						<div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
							<button
								...
								onClick={() => {
									...
									} else localStorage.setItem("cart", JSON.stringify([91]));
									setModalOpen(false);
								}}
							>
								Add to Cart
